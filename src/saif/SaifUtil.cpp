...
#define TType saifParser::token

saif::SaifLexer::SaifLexer(std::istream *i) : istm(i), m_string("") {
  state.push_back(pair<unsigned int, unsigned int>(S_BEGIN, 0));
}
...
bool saif::SaifLexer::token_helper(int vrvt, bool push_state,
                                   unsigned int new_state,
                                   unsigned int sub_state, bool pop_state,
                                   int &rvt, bool breturn) {
  if(push_state)
...
bool saif::SaifLexer::validate_token(const string &t, saif_token_type *tt,
                                     int &rvt) {
  switch(state.back().first) {
  case S_BEGIN: {
    if(t == "INSTANCE")
      return token_helper(TType::SKeyINSTANCE, true, S_INST, 0, false, rvt,
                          true);
    if(t == "(")
      return token_helper('(', true, S_BEGIN, 0, false, rvt, true);
    if(t == "SAIFILE")
      return token_helper(TType::SKeySAIFILE, true, S_BEGIN, 0, false, rvt,
                          true);
    if(t == "SAIFVERSION")
      return token_helper(TType::SKeySAIFVERSION, true, S_DEF, 0, false, rvt,
                          true);
    if(t == "DIRECTION")
      return token_helper(TType::SKeyDIRECTION, true, S_DEF, 0, false, rvt,
                          true);
    if(t == "DESIGN")
    ...
    if(t == "PROGRAM_NAME")
      return token_helper(TType::SKeyPROGRAM_NAME, true, S_DEF, 0, false, rvt,
                          true);
    if(t == "VERSION")
    ...
    if(t == "TIMESCALE")
      return token_helper(TType::SKeyTIMESCALE, true, S_TIMESCALE, 0, false,
                          rvt, true);
    if(t == "DURATION")
      return token_helper(TType::SKeyDURATION, true, S_DUR, 0, false, rvt,
                          true);
    if(t == ")")
